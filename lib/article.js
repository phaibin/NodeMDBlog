...
Object.defineProperty(Article.prototype, 'searchSummary', { get: function() {
  if (this.searchContent != undefined && this.searchContent.length > 0) {
    return marked(this.searchContent);
  } else {
    return this.summary;
  }
}});
...
Article.prototype.match = function(keywords) {
  ...
  keywords.forEach(function(keyword) {
    if (that.title.toLowerCase().indexOf(keyword.toLowerCase()) !== -1) {
      ...
      that.searchTitle = that.searchTitle.replace(re, '<span class=\"search\">'+keyword+'</span>');
    }
    if (that.body.toLowerCase().indexOf(keyword.toLowerCase()) !== -1) {
      find = true;
      var re = new RegExp('(.*)'+keyword+'(.*)', 'i');
      that.searchContent = (re.exec(that.body))[0];
      var re2 = new RegExp(keyword, 'ig');
      that.searchContent = that.searchContent.replace(re2, '<span class=\"search\">'+keyword+'</span>');
      that.searchContent = that.searchContent + '  \n...\n\n';
    }
  });
  this.searchContent = marked(this.searchContent);
  return find;
};

Article.load = function() {
  console.log('load');
  var allFiles = fs.readdirSync(__dirname+'/../views/articles').filter(function(file) {
    return file.substr(-3) === '.md';
  });
  allFiles = allFiles.map(function(f) { return __dirname+'/../views/articles/' + f; });
